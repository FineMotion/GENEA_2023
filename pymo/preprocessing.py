...
class MocapParameterizer(BaseEstimator, TransformerMixin):
    def __init__(self, param_type='euler'):
        ...
        param_type = {'euler', 'quat', 'expmap', 'position', 'expmap2pos', 'ortho6d'}
        '''
    ...
    def transform(self, X, y=None):
        ...
        elif self.param_type == 'ortho6d':
            return self._to_ortho6d(X)
    ...
    def inverse_transform(self, X, copy=None):
        if self.param_type == 'euler':
            ...
            return self._ortho6d_to_euler(X)
        else:
    ...
    def _to_ortho6d(self, X):
        '''Converts Euler angles to 6D continuous representation'''
        Q = []
        for track in X:
            euler_df = track.values

            ortho_df = euler_df.copy()  # type: pd.DataFrame
            ...
            for joint in joints:
                ...
                ortho_df.drop([r1_col, r2_col, r3_col], axis=1, inplace=True)
                euler = [[f[1][r1_col],
                          f[1][r2_col],
                          f[1][r3_col]] for f in r.iterrows()]
                rotmats = R.from_euler(rot_order.lower(), euler, degrees=True).as_matrix()  # shape: Nx3x3
                a1 = rotmats[:, :, 0]  # shape: Nx3
                a2 = rotmats[:, :, 1]  # shape: Nx3

                ortho_df.insert(loc=0, column=f'{joint}_a1x',
                                value=pd.Series(data=[e[0] for e in a1], index=ortho_df.index))
                ortho_df.insert(loc=0, column=f'{joint}_a1y',
                                value=pd.Series(data=[e[1] for e in a1], index=ortho_df.index))
                ortho_df.insert(loc=0, column=f'{joint}_a1z',
                                value=pd.Series(data=[e[2] for e in a1], index=ortho_df.index))
                ortho_df.insert(loc=0, column=f'{joint}_a2x',
                                value=pd.Series(data=[e[0] for e in a2], index=ortho_df.index))
                ortho_df.insert(loc=0, column=f'{joint}_a2y',
                                value=pd.Series(data=[e[1] for e in a2], index=ortho_df.index))
                ortho_df.insert(loc=0, column=f'{joint}_a2z',
                                value=pd.Series(data=[e[2] for e in a2], index=ortho_df.index))

            new_track = track.clone()
            new_track.values = ortho_df
            Q.append(new_track)

        return Q

    def _ortho6d_to_euler(self, X):
        Q = []
        for track in X:
            ortho_df = track.values  # type: pd.DataFrame

            euler_df = ortho_df.copy()  # type: pd.DataFrame

            ortho_params = [c for c in ortho_df.columns if (any(p in c for p in [
                'a1x', 'a1y', 'a1z', 'a2x', 'a2y', 'a2z']) and 'Nub' not in c)]

            joints = [joint for joint in track.skeleton if 'Nub' not in joint]

            for joint in joints:
                r = ortho_df[[c for c in ortho_params if joint in c]]

                euler_df.drop([f'{joint}_a1x', f'{joint}_a1y', f'{joint}_a1z',
                               f'{joint}_a2x', f'{joint}_a2y', f'{joint}_a2z'], axis=1, inplace=True)
                a1 = np.array([[
                    f[1][f'{joint}_a1x'], f[1][f'{joint}_a1y'], f[1][f'{joint}_a1z']
                ] for f in r.iterrows()])  # Nx3

                a2 = np.array([[
                    f[1][f'{joint}_a2x'], f[1][f'{joint}_a2y'], f[1][f'{joint}_a2z']
                ] for f in r.iterrows()])  # Nx3
                ...
                rotmats = np.concatenate([x, y, z], axis=-1)
                rot_order = track.skeleton[joint]['order']
                euler_rots = np.array(R.from_matrix(rotmats).as_euler(rot_order.lower(), degrees=True))

                euler_df[f'{joint}_{rot_order[0]}rotation'] = pd.Series(data=[e[0] for e in euler_rots], index=euler_df.index)
                euler_df[f'{joint}_{rot_order[1]}rotation'] = pd.Series(data=[e[1] for e in euler_rots], index=euler_df.index)
                euler_df[f'{joint}_{rot_order[2]}rotation'] = pd.Series(data=[e[2] for e in euler_rots], index=euler_df.index)
...
class JointSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for joint_name in selected_joints:
            reg = re.compile(f'{joint_name}_[XYZ].+')
            selected_channels.extend([o for o in X[0].values.columns if reg.match(o) is not None])
