...
#        return X
    ...
    def _to_quat(self, X):
        ...
        for track in X:
            ...
            # Create a new DataFrame to store the Quaternion rep
            ...
            for joint in joints:
                ...
                euler = [[f[1][r1_col], f[1][r2_col], f[1][r3_col]] for f in r.iterrows()]
                quats = np.array(R.from_euler(rot_order.lower(), euler, degrees=True).as_quat())

                # Create the corresponding columns in the new DataFrame    
                quat_df.insert(loc=0, column='%s_a_quat'%joint, value=pd.Series(data=[q[0] for q in quats], index=quat_df.index))
                quat_df.insert(loc=0, column='%s_b_quat'%joint, value=pd.Series(data=[q[1] for q in quats], index=quat_df.index))
                quat_df.insert(loc=0, column='%s_c_quat'%joint, value=pd.Series(data=[q[2] for q in quats], index=quat_df.index))
                quat_df.insert(loc=0, column='%s_d_quat'%joint, value=pd.Series(data=[q[3] for q in quats], index=quat_df.index))
    ...
    def _quat_to_euler(self, X):
        ...
        for track in X:
            ...
            # List the columns that contain rotation channels
            quat_params = [c for c in quat_df.columns if ( any(p in c for p in ['a_quat', 'b_quat','c_quat', 'd_quat']) and 'Nub' not in c)]
            ...
            for joint in joints:
                ...
                euler_df.drop(['%s_a_quat'%joint, '%s_b_quat'%joint, '%s_c_quat'%joint, '%s_d_quat'%joint], axis=1, inplace=True)
                quat = [[f[1]['%s_a_quat'%joint], f[1]['%s_b_quat'%joint], f[1]['%s_c_quat'%joint], f[1]['%s_d_quat'%joint]] for f in r.iterrows()] # Make sure the columsn are organized in xyz order
                rot_order = track.skeleton[joint]['order']
                euler_rots = np.array(R.from_quat(quat).as_euler(rot_order.lower(), degrees=True))
...
class JointSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for joint_name in selected_joints:
            selected_channels.extend([o for o in X[0].values.columns if ("_" + joint_name + "_") in o and 'Nub' not in o])
