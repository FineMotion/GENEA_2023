...
def get_gestures(audio_main_path):
    ...
    for i in range(len(split_audio_first)):
        beats_first = get_beats(split_audio_first[i], len(split_gestures_first[i]), False)
        ...
        if len(split_audio_second) > i:
            beats_second = get_beats(split_audio_second[i], len(split_gestures_second[i]), False)
